...
**Skill ID:** SKILL_conceptual_design
**Commands:** `/abs`, `/fn`, `/morpho`, `/eval`, `/vs`
...
|---------|---------|---------|
| `/abstract <problem>` | `/abs` | 5-step abstraction process |
| `/functions` | `/fn`, `/taofunction` | Function structure breakdown |
| `/morpho` | `/matrix`, `/taomorpho` | Morphological matrix |
| `/evaluate` | `/eval`, `/danhgia` | VDI 2225 concept evaluation |
| `/compare <A> <B>` | `/vs` | Side-by-side concept comparison |

### Command Output Templates

**`/abstract`** → Abstraction analysis:
```markdown
| Step | Transformation | Result |
|------|----------------|--------|
| 1. Remove preferences | "Use servo" | "Convert to motion" |
| 5. Essence | — | Transform [IN] → [OUT] |
```

**`/functions`** → Function structure:
```markdown
| ID | Function | Input | Output | Type |
|----|----------|-------|--------|------|
| F1 | Convert energy | Elec | Mech | Main |
```
...
| Function | P1 | P2 | P3 |
|----------|----|----|-----|
| F1 | Motor | Hydraulic | Pneumatic |
```

**`/evaluate`** → VDI 2225 scoring (0-4 scale):
```markdown

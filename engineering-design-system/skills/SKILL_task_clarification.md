...
**Skill ID:** SKILL_task_clarification
**Commands:** `/req`, `/validate`, `/mil`, `/stake`
...
|---------|---------|---------|
| `/requirements` | `/req`, `/taoreq` | Generate requirements list (16 categories) |
| `/validate` | `/check` | Check completeness & conflicts |
| `/standards` | `/mil`, `/std` | Map MIL-STD to requirements |
| `/stakeholders` | `/stake` | Stakeholder analysis |

### Command Output Templates

**`/requirements`** → Creates `01_requirements/requirements_list.md`:
```markdown
...
**`/validate`** → Generates validation report:
```markdown
## Completeness Score: [XX]%
| Check | Status | Details |
|-------|--------|---------|
| All categories | ✅ | 16/16 |
| Quantified | 🟡 | 75% (target: 80%) |
```

**`/standards`** → Creates standards mapping:
```markdown
| Requirement | Standard | Clause | Compliance |
|-------------|----------|--------|------------|
| R-07-01 | MIL-STD-810H | 501.7 | Test |
```
